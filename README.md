#Скрипт для резервного копирования конфигураций роутеров на MikroTik RouterOS в Docker

***
##Особенности:
* Работает из под Docker
...
**Внимание. Это экспериментальная версия скрипта.**  
У меня есть возможность тестировать его работу только на ~40 устройствах Mikrotik.  
В случае возникновения проблем вы можете откатиться на предыдущую версию.


##Базовая настройка
1. Для работы скрипта необходимо установить: Окружение Docker и Docker-compose

2. Переключаемся на свежесозданного пользователя и копируем себе скрипт  
	`su - mt`  
	`git clone https://github.com/0x566164696D/mt_backup_ng.git`  
	`cd mt_backup_ng`
3. Сгенерить ключи для SSH .

4. Выполняем настройку скрипта, редактируем файл dokcer-compose.yml
...
2. Прописываем в конфигурационном файле config.conf путь до свежесозданного приватного ключа (по умолчанию ~/.ssh/id_rsa) и логин, под которым мы впоследствии будем подключаться к роутерам.
...
Обратите внимание, что в config.conf появилась новая опция - **threads**. 
